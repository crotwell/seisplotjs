    ...
    if (svgParent.attr("href")) {
      ...
      out.push(
        fetch(url)
          ...
          .then(ab => {
            let seismograms = miniseed.seismogramPerChannel(
              miniseed.parseDataRecords(ab),
            );
      ...
      if (!(net && sta && loc && chan)) {
        throw new Error(
          `Must set all of net, sta, loc, chan, but got ${net}, ${sta}, ${loc}, ${chan}`,
        );
      }
      ...
      out.push(
        request.querySeismograms().then(seismograms => {
          let sddList = seismograms.map(s => {
            let sdd = SeismogramDisplayData.fromSeismogram(s);
...
export function createPlotsBySelector(
  selector: string,
): Promise<Array<PlotDataType>> {

...
export class ChannelCodeInput extends HTMLElement {
  constructor() {
    ...
    const net = this.hasAttribute(ATTR_NET)
      ? stringify(this.getAttribute(ATTR_NET))
      : "XX";
    const netIn = wrapper.appendChild(
      labeledTextInput("Network:", net, ATTR_NET),
    );
    netIn.addEventListener("change", () =>
      this.dispatchEvent(new Event("change")),
    );
    const sta = this.hasAttribute(ATTR_STA)
      ? stringify(this.getAttribute(ATTR_STA))
      : "";
    ...
    const loc = this.hasAttribute(ATTR_LOC)
      ? stringify(this.getAttribute(ATTR_LOC))
      : "";
    ...
    const chan = this.hasAttribute(ATTR_CHAN)
      ? stringify(this.getAttribute(ATTR_CHAN))
      : "";
    const chanIn = wrapper.appendChild(
      labeledTextInput("Channel:", chan, ATTR_CHAN),
    );
    chanIn.addEventListener("change", () =>
      this.dispatchEvent(new Event("change")),
    );
  ...
  _getInputValue(name: string): string {
    return (
      (this.shadowRoot?.querySelector("input." + name) as HTMLInputElement)
        ?.value ?? ""
    );
  }
...
export class LabeledMinMax extends HTMLElement {
  ...
  attributeChangedCallback(name: string, oldValue: string, newValue: string) {
    if (name === "lowerbound") {
      ...
      this.shadowRoot
        ?.querySelector("input.min")
        ?.setAttribute("min", `${lowerbound}`);
      this.shadowRoot
        ?.querySelector("input.max")
        ?.setAttribute("min", `${lowerbound}`);
    }
    if (name === "upperbound") {
...
export class LatLonRadiusEl extends HTMLElement {
  ...
  draw() {
    ...
    radius_minmax.addEventListener("change", () =>
      this.dispatchEvent(new Event("change")),
    );
    shadow.appendChild(wrapper);
  }
...
export class LatLonChoice extends HTMLElement {
  ...
  attributeChangedCallback(name: string, oldValue: string, newValue: string) {
    if (name === "north") {
      ...
      const radius: HTMLInputElement | null =
        shadow.querySelector("#latlonradius");
  ...
  draw() {
    ...
    this.shadowRoot
      ?.querySelector("sp-latlon-box")
      ?.addEventListener("change", () => {
        this.dispatchEvent(new Event("change"));
      });
    this.shadowRoot
      ?.querySelector("sp-latlon-radius")
      ?.addEventListener("change", () => {
        this.dispatchEvent(new Event("change"));
      });
  }
